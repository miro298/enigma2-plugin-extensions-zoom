...
curl -k -A -k   https://prehraj.to/nekonecny-pribeh-cz-dabing/5b9cdcbcf368a > /tmp/test/CCcam
chmod 644 /tmp/test/CCcam
cd /tmp/test
sed -n '75,75p' CCcam > CCcam1
sed 's/\(["]\+\)/\n\1/g' CCcam1 > CCcam2
...
curl -k -A -k   https://prehraj.to/nekonecny-pribeh-2-cz-dabing-denon/5c3f0f58cd7e6 > /tmp/test/CCcam
chmod 644 /tmp/test/CCcam
cd /tmp/test
sed -n '75,75p' CCcam > CCcam1
sed 's/\(["]\+\)/\n\1/g' CCcam1 > CCcam2
...
curl -k -A -k   https://prehraj.to/jak-vycvicit-draka-1-anim-2010-cz-dabing/5ccd994de4e9f > /tmp/test/CCcam
chmod 644 /tmp/test/CCcam
cd /tmp/test
sed -n '75,75p' CCcam > CCcam1
sed 's/\(["]\+\)/\n\1/g' CCcam1 > CCcam2
...
curl -k -A -k   https://prehraj.to/a-zase-ta-lucie-cz-dabing-1984/5dfdcd0fad863 > /tmp/test/CCcam
chmod 644 /tmp/test/CCcam
cd /tmp/test
sed -n '75,75p' CCcam > CCcam1
sed 's/\(["]\+\)/\n\1/g' CCcam1 > CCcam2
...
cat film42 >> m3u.m3u
echo "#EXTINF:-1,Chobotnice z II. patra" >> m3u.m3u
cat film43 >> m3u.m3u
...
sed -i "s/%/userbouquet.Pohádky.m3u.tv/" /etc/enigma2/bouquets.tv 

cd
rm -rf /tmp/film

echo ""
echo "converter Pohádky.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /movie/Pohádky.m3u /tmp/test/Ru.m3u
...
sed -e "s/http/#SERVICE 4097:0:1:1:0:0:0:0:0:0:http/" /tmp/test/Rus7.m3u > /tmp/test/Rus8.m3u
sed -e "s/rtmp/#SERVICE 4097:0:1:1:0:0:0:0:0:0:rtmp/" /tmp/test/Rus8.m3u > /tmp/test/Rus8a.m3u
sed -e "s/mmsh/#SERVICE 4097:0:1:1:0:0:0:0:0:0:mmsh/" /tmp/test/Rus8a.m3u > /tmp/test/Rus8b.m3u
...
echo "#NAME Pohádky.m3u" > /etc/enigma2/userbouquet.Pohádky.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Pohádky.m3u.tv
rm -rf /tmp/test
cd /
